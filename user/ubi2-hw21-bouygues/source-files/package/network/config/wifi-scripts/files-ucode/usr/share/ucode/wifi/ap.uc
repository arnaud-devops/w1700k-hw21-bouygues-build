...
export function generate(interface, data, config, vlans, stas, phy_features) {
	...
	if (data.config.band == '6g') {
		...
		/* W1700K: 6 GHz-only SSID in-band discovery so clients can re-find
		 * the AP quickly after sleep/wake (no same-SSID RNR co-location). */
		append('unsol_bcast_probe_resp_interval', 20);
	}
